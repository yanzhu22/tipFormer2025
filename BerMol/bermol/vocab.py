...
class MolVocab:
    ...
    def build_corpus(self, mol_path: str, corpus_path: str = None, update: bool = True) -> "MolVocab":
        ...
        corpus_res = Parallel(n_jobs=1)(  # Disable parallel to see debug output
        ...
        with open(corpus_path, "w") as f_cor:
            for data in tqdm(corpus_res):
                ...
                print(f"Original descriptor length: {len(desc)}")
                # Ensure descriptor length matches RDKit's 208 descriptors
                if len(desc) != len(self.descriptor):
                    print(f"Adjusting descriptor length from {len(desc)} to {len(self.descriptor)}")
                    desc = desc[:len(self.descriptor)] + [0.0]*(len(self.descriptor) - len(desc)) if len(desc) < len(self.descriptor) else desc[:len(self.descriptor)]
                if len(sentence) > self.max_len:
                    print(sentence)
                # Ensure descriptor length matches
                if len(desc) != len(self.descriptor):
                    # Pad with zeros if shorter, truncate if longer
                    desc = desc[:len(self.descriptor)] + [0.0]*(len(self.descriptor) - len(desc)) if len(desc) < len(self.descriptor) else desc[:len(self.descriptor)]
                f_cor.write(
                    "\t".join([" ".join(sentence), " ".join(fgs), " ".join(map(str, desc))])
                    + "\n"
                )
                cnt += 1
